...
export async function run(page: Page, params: {}) {
    ...
    /* STEP: Click the 'View History' link to modify the menu. */
    const viewHistoryLink = page.getByRole('link', {
        name: 'View history',
    });
    await viewHistoryLink.click();

    /* STEP: Assert the latest edit was made by the user 'Maxeto0910'. */
    const latestEditName = page
        .getByRole('link', { name: 'Maxeto0910' })
        .first();
    expect(latestEditName).toHaveText('Maxeto0910');
}
